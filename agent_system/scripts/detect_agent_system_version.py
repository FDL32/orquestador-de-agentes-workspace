...
class AgentSystemDetector:
    ...
    MARKERS = {
        ...
        "v9.0-v9.1": {
            "required": [".agent/agent_controller.py", ".agent/rules/", "skills/", "CLAUDE.md"],
    ...
    def check_markers(self, version: str) -> Tuple[bool, Dict]:
        ...
        required_met = all(
            (self.project_path / marker).exists()
            for marker in markers["required"]
        )

        optional_met = sum(
            1 for marker in markers["optional"]
            if (self.project_path / marker).exists()
        )

        absent_violated = any(
            (self.project_path / marker).exists()
            for marker in markers["absent"]
        )
    ...
    def detect_version(self) -> Dict:
        ...
        results = {}
        for version in ["v9.2.1+", "v9.2", "v9.0-v9.1", "v8.x"]:
    ...
    def suggest_upgrade_path(self, detected_version: str) -> str:
        ...
        paths = {
            "v8.x": "v8.x â†’ v9.0-v9.1 â†’ v9.2 â†’ v9.2.1+",
            "v9.0-v9.1": "v9.0-v9.1 â†’ v9.2 â†’ v9.2.1+",
            "v9.2": "v9.2 â†’ v9.2.1+",
            "v9.2.1+": "Already latest",
        }
...
def main():
    ...
    if result["detected"]:
        ...
        if "details" in result:
            ...
            print(f"  Optional markers: {details.get('optional_met')}/{details.get('optional_total')}")
