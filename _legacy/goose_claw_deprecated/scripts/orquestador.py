"""
Orquestador multi-agente v2.5 (Multi-Engine Support)
    ...
    try:
        ...
        try:
            try:
                ...
                # Intentar fallback automÃ¡tico al otro motor
                alternative = "goose" if engine_name == "claw" else "claw"
                print(
                    f"\n[WARN] FALLBACK: '{engine_name}' no encontrado. Intentando con '{alternative}'..."
                )
                adapter = ADAPTERS[alternative]
                engine_name = alternative
                result = adapter.run(prompt_ref, env=os.environ.copy())

        except FileNotFoundError:
            print("\n[ERROR] Ni 'goose' ni 'claw' estan disponibles en el PATH.")
            return 127
