...
> Todos los tickets vivos restantes ya tienen ficha congelable: solo `011h` (pendiente de review). `011i` (y `013b` absorbido) cerrados: el default xdist no es perseguible sin reescribir 3 tests con estado global (ver `CG-WOT-2026-011i.md`). `002c` y `256a` quedan fuera por naturaleza (`completed-partial` y `blocked` externo).
