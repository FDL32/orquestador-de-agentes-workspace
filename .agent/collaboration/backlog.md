...
- **Depende de:** -
- **Reactivation:** -
- **Origen:** session-2026-06-22-close-audit (promovido 2026-06-25 como siguiente follow-up de menor riesgo tras cerrar `013u`).
- **Problema:** `repo_destino/.agent/runtime/reviews/`, `repo_destino/.agent/runtime/review_packets/` y `repo_destino/.agent/runtime/memory/observations.jsonl.bak.*` crecen sin techo en DISCO local. Estan gitignored y excluidos de manifiestos portables: no contaminan repo ni publicacion, pero hoy no existe una via auditable y de bajo riesgo para acotar su retencion.
- **Objetivo:** introducir una retencion local, opt-in y auditable por conteo/edad para esas 3 superficies gitignored, sin integrarla aun en `session-close` ni tocar historico versionado/util (`events/archive`, `audits/system_health`, `_archive/plan_audit`, `collaboration/archive`).
- **Contrato canonico (FUENTE UNICA):** `.agent/planning/work_plan_WOT-2026-013l.md`.
  El packet fija premise re-check, FLT, superficies prohibidas, bateria focal y DoD binario. Esta ficha es solo indice.
