...
#### R-006: For operational launch, resume, and recovery decisions, derive ticket state from...[truncated]
...
#### R-007: In motor+destino topology, operational backlog.md lives in repo_destino/.agent/c...[truncated]
...
#### R-008: Local runtime cleanup and bus reconciliation are different recovery classes. If...[truncated]
...
#### R-009: Recovery paths must be idempotent against the primary mechanism. Before emitting...[truncated]
...
#### R-010: When a critical bus trigger outlives its main consumer, the durable fix is to en...[truncated]
...
#### R-015: This system starts every plan with a base `...a` ticket; `...b` and later letter...[truncated]
...
#### R-016: Ticket WOT-2026-003d completado: ** El residue-prune del instalador nunca borra...[truncated]
...
#### R-019: Ticket WT-2026-251a completado: ** Centralizar ticket-ID regex y extender a pref...[truncated]
...
#### R-022: Any regex, import path, literal string, or code snippet inside a plan acts as ex...[truncated]
...
#### R-023: Before closing a technical plan, verify proposed test file paths against the fil...[truncated]
...
#### R-024: Before freezing 'Tests Esperados' in a plan or audit, verify whether each named...[truncated]
...
#### R-025: For concurrency or cross-process coordination fixes, passing tests is necessary...[truncated]
...
#### R-026: For one-line fixes with high regression risk, the minimum useful contract is sma...[truncated]
...
#### R-027: The Manager is not an untouchable narrator layer. If the fault lives in review i...[truncated]
...
#### R-028: The strongest review role is an auditor who searches for counterexamples in the...[truncated]
...
#### R-029: When a planning correction lands during review, apply it to both `work_plan.md`...[truncated]
...
#### R-001: Cross-process requeue authority should be a real atomic claim keyed by (ticket_i...[truncated]
...
#### R-002: Files Likely Touched must use paths relative to the motor git repo, in the same...[truncated]
...
#### R-003: When Builder can declare ready without implementation evidence, Manager spends r...[truncated]
...
#### R-004: When the implementation is already committed, Builder's job is not to reimplemen...[truncated]
...
#### R-005: When the implementation is already verified but the bus or session-close path dr...[truncated]
...
#### R-011: After the canonical session-close pipeline runs on a tracked destination, it can...[truncated]
...
#### R-012: El cierre canonico no valida que el ultimo commit del ticket tenga mensaje descr...[truncated]
...
#### R-013: If two BUILDER_RELAUNCH_ATTEMPTED events for the same ticket have different roun...[truncated]
...
#### R-014: In this repo_destino, keeping a fresh `.agent/runtime/audit/AUDIT.md` after cano...[truncated]
...
#### R-020: UTF-8 with BOM can make lightweight validators and Windows subprocess readers fa...[truncated]
...
#### R-021: When a ticket is re-scoped or closed through follow-up reviews, the backlog tabl...[truncated]
...
#### R-030: For validators with multiple failure modes, keep tests orthogonal: each test sho...[truncated]
